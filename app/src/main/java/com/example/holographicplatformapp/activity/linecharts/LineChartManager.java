...
public class LineChartManager {
     ...
     * @Description:创建两条折线
     */
    public static LineData initDoubleLineChart(Context context, LineChart mLineChart, int count, int[] datas1, int[] datas2, List<String> mListDate) {

        ArrayList<String> xValues = new ArrayList<String>();
        for (int i = 0; i < mListDate.size(); i++) {
            // x轴显示的数据，这里默认使用数字下标显示
            xValues.add(mListDate.get(i));
        }
